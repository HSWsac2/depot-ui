...
export const DepotContextProvider = ({ children }) => {
	...
	const [depotCookie, setDepotCookie, removeDepotCookie] = useCookies([
		"depot",
	]);
	useEffect(() => {
		...
		axios
			.get(
				process.env.REACT_APP_BACKEND_URL_DEPOT_SERVICE +
					`depots/${positionId}/${positionSubId}`
			)
	...
	const selectDepot = useCallback(
		(depot, rememberDepot) => {
			...
			if (rememberDepot) {
				setDepotCookie(
					positionIdcookieName,
					depot.position_id,
					defaultCookieOptions
				);
				setDepotCookie(
					positionSubIdcookieName,
					depot.position_sub_id,
					defaultCookieOptions
				);
			}
		},
